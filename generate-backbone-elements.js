...
// Get all BackboneElement types
function getBackboneElements() {
...
// Generate class for a BackboneElement
function generateBackboneElementClass(interfaceName) {
...
import { BackboneElement } from '../base/BackboneElement';`;
  ...
  sortedImports.forEach(imp => {
    if (imp !== 'BackboneElement') {
      if (['Element', 'DomainResource', 'Resource'].includes(imp)) {
        classContent += `\nimport { ${imp} } from '../base/${imp}';`;
      } else if (imp.endsWith('BackboneElement') || imp.includes('BackboneElement')) {
        classContent += `\nimport { ${imp} } from './${imp}';`;
      } else {
        // Try elements first
  ...
  // Class definition
  classContent += `/**\n * ${interfaceComment || interfaceName}\n */\nexport class ${interfaceName} extends BackboneElement implements fhir.${interfaceName} {\n`;
  
  // Generate properties (limit to 30 to keep files manageable)
...
// Get all BackboneElement types
const backboneElements = getBackboneElements();
console.log(`Found ${backboneElements.length} BackboneElement types`);

// Generate classes in batches to avoid overwhelming the system
const batchSize = 50;
const totalBatches = Math.ceil(backboneElements.length / batchSize);

for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
  const startIndex = batchIndex * batchSize;
  const endIndex = Math.min(startIndex + batchSize, backboneElements.length);
  const batch = backboneElements.slice(startIndex, endIndex);
  
  console.log(`\\nGenerating batch ${batchIndex + 1}/${totalBatches} (${batch.length} classes)...`);
  
  batch.forEach(elementType => {
    const classContent = generateBackboneElementClass(elementType);
    if (classContent) {
      const filePath = path.join('src', 'backbone', `${elementType}.ts`);
...
console.log(`\\nGenerated ${backboneElements.length} BackboneElement classes!`);